...
class _TransactionDb:
    ...
    def query(self, sql, params=None):
        if "is_best_debater=TRUE" in sql:
            rows = [] if not self.existing_best else [{"chapter_label": self.existing_best}]
...
def _chapter(label, time_text="1:23", enabled=True):
    return {"chapter_label": label, "enabled": enabled, "time_text": time_text}
...
def test_save_chapters_marks_exactly_one_enabled_individual_role():
    db = _TransactionDb()
    result = media_logic.save_chapters(
        7,
        [_chapter("正主"), _chapter("自由辯論", "9:00")],
        best_debater_role="正主",
        db=db,
    )
    assert result["ok"] is True
    insert = next(params for sql, params in db.connection.calls if "INSERT INTO video_chapters" in sql)
    assert [row["chapter_label"] for row in insert if row["is_best_debater"]] == ["正主"]
...
def test_bootstrap_and_versioned_migration_enforce_one_best_and_roster_roles():
    ...
    migration = next(item for item in migrations if item.version == "20260714_0006")
    assert "idx_video_chapters_one_best_debater" in migration.up_sql
    assert "video_chapters_best_role_check" in migration.up_sql
    assert "REVOKE ALL PRIVILEGES ON TABLE video_roster" in migration.up_sql
