...
回覆要精簡、可執行，第一行必須先寫「結論：通過／需要修改／不建議加入」，之後最多列 4 點最重要理由或修改建議。"""

VOTE_DISCUSSION_SYSTEM_PROMPT = """你是辯題討論區的 AI 助手。請使用粵語書面語，保持中立，集中分析議案本身，不要代替成員投票。

回覆要簡潔到位，第一行必須先寫「結論：...」，全文控制在 150 字內；之後最多列 3 點主要爭議、可補充資料或正反角度。"""


def build_vote_topic_review_prompt(topic: str, category: str, difficulty_label: str) -> str:
    return f"""待審查辯題：{topic}
類別：{category}
難度：{difficulty_label}
...
def build_vote_discussion_prompt(motion_type: str, motion_key: str, discussion_lines: list[str]) -> str:
