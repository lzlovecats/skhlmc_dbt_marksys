...
def _run_status_checks() -> dict:
    ...
    try:
        ...
        results["errors"].append(f"Table row counts failed: {e}")
    ...
    try:
        ...
        results["errors"].append(f"Telegram queue check failed: {e}")
    ...
    try:
        ...
        results["errors"].append(f"system_config check failed: {e}")
    ...
    try:
        ...
        results["errors"].append(f"voting info check failed: {e}")
    ...
    try:
        ...
        results["errors"].append(f"login record check failed: {e}")
...
def _render_status_results(results: dict):
    if results["db_ok"]:
        ...
        st.error(f"Error during connection: {results['db_error']}")
        return

    if results["table_counts"] is not None:
        counts = results["table_counts"]
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("帳戶數量", counts.get("accounts", "—"))
    ...
    c5.metric("Telegram 待處理訊息", tg_queue_depth if tg_queue_depth is not None else "—")
    ...
    if results["config_admin_ok"]:
        st.success("admin_password was set")
    else:
        st.warning("在system_config中找不到admin_password")
    if results["config_developer_ok"]:
        st.success("developer_password was set")
    else:
        st.warning("Failed to find developer_password in system_config")

    if results["errors"]:
        st.warning("Some checking failed：\n" + "\n".join(f"- {e}" for e in results["errors"]))


# ─── Page header ──────────────────────────────────────────────────────────────

st.title("聖呂中辯電子分紙系統")
st.caption("歡迎使用skhlmc_dbt_marksys！")
st.caption("如發現任何錯誤或希望提出改進建議，請在[GitHub](https://github.com/lzlovecats/skhlmc_dbt_marksys/issues)"
           "提交Issue或直接聯絡開發者。")
