-- A member's participation window starts at accounts.active_since.  The old
-- view applied that boundary to eligible motions and participation counts, but
-- not to total_ballots / agree_ballots.  That made new members' agree-rate
-- denominators include ballots from before their participation start date.
...
combined_ballots AS (
    SELECT b.user_id, b.vote_choice, tv.created_at
    FROM topic_vote_ballots b
    JOIN topic_votes tv ON tv.topic_text = b.topic_text
    UNION ALL
    SELECT b.user_id, b.vote_choice, tdv.created_at
    FROM topic_removal_vote_ballots b
...
ballot_summary AS (
    SELECT
        ...
        COUNT(cb.vote_choice) FILTER (
            WHERE cb.vote_choice = 'agree'
        ) AS agree_ballots
    FROM accounts a
    LEFT JOIN combined_ballots cb
     ...
     AND (
         ...
         OR cb.created_at::date >= a.active_since
     )
    GROUP BY a.user_id
),
base_stats AS (
