...
with col_info3:
    st.write(f"**提交時間（UTC）：** {judge_record['mark_time']}")
...
with col_pro:
    ...
    st.write(f"主辯分數：{judge_record['pro1_m']} / 100 ")
    st.write(f"一副分數：{judge_record['pro2_m']} / 100")
    st.write(f"二副分數：{judge_record['pro3_m']} / 100")
    st.write(f"結辯分數：{judge_record['pro4_m']} / 100")
    st.write(f"正方自由辯論總分：{judge_record['pro_free']} / 55 ")
    st.write(f"正方扣分總和：{judge_record['pro_deduction']}")
    st.write(f"正方內容連貫：{judge_record['pro_coherence']} / 5 ")
    st.divider()
    st.metric("正方總分", f"{judge_record['pro_total']}／460 ")

with col_con:
    st.subheader(f"反方：{judge_record['con_name']}")
    st.write(f"主辯分數：{judge_record['con1_m']} / 100 ")
    st.write(f"一副分數：{judge_record['con2_m']} / 100 ")
    st.write(f"二副分數：{judge_record['con3_m']} / 100 ")
    st.write(f"結辯分數：{judge_record['con4_m']} / 100 ")
    st.write(f"反方自由辯論總分：{judge_record['con_free']} / 55 ")
    st.write(f"反方扣分總和：{judge_record['con_deduction']}")
    st.write(f"反方內容連貫：{judge_record['con_coherence']} / 5 ")
    st.divider()
