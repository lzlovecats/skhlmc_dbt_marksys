...
if pro_total > con_total:
    winner_label = f"正方 ({judge_record['pro_team']})"
elif con_total > pro_total:
    winner_label = f"反方 ({judge_record['con_team']})"
else:
    winner_label = "平局"
sum_col1, sum_col2 = st.columns(2)
sum_col1.metric(f"正方總分（{judge_record['pro_team']}）", f"{pro_total} / {GRAND_TOTAL}")
sum_col2.metric(f"反方總分（{judge_record['con_team']}）", f"{con_total} / {GRAND_TOTAL}")
...
def display_team_scores(side_label, team_name, record, detail_a, detail_b):
    ...
    if side_label == "正方":
        ...
        deduction_key, coherence_key, total = "con_deduction_points", "con_coherence_score", "con_total_score"
