...
| Mode A真人聯機完場AI評判 | `deploy/proxy.py`, `frontend/shared/room-debate-p2p.js` | 完場後只由主持手動要求一次；正反雙方各有Browser逐字稿時，依次 Gemini 3.5 Flash → Gemini 2.5 Flash → Gemini 2.5 Flash Lite；不另設output token上限，但保留2MiB回應上限及45秒timeout；P2P音訊及委員帳號不送provider | `AI_FEATURE_MODEL_FALLBACK_LABELS["room_judgement"]` |
