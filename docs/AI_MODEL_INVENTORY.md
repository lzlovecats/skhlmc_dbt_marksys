...
| 聯機多人對 AI | `deploy/proxy.py`, `templates/room_debate.html` | 同一 Gemini Live 模型 | 同上 |
| 聯機完場 AI 評判 | `deploy/proxy.py` | 依次 Gemini 3.5 Flash → Gemini 2.5 Flash → Gemini 2.5 Flash Lite | `AI_FEATURE_MODEL_FALLBACK_LABELS["room_judgement"]` |
| Kiosk 全場錄音評語及建議勝方 | `api/kiosk_api.py`, `templates/appliance_practice.html` | Gemini 3.5 Flash（Gemini-only，必須支援 audio） | `AI_FEATURE_MODEL_LABELS["kiosk_match_review"]`, `AI_FEATURE_REQUIREMENTS` |
...
| 功能 | 實作位置 | 模型／引擎管理方式 |
| --- | --- | --- |
| Azure TTS | `deploy/proxy.py` | 聲音由 `TTS_PROVIDER_OPTIONS["azure"]` 集中定義 selector 及預設 `zh-HK-HiuMaanNeural`；部署可用 `AZURE_TTS_VOICE` 覆寫 |
