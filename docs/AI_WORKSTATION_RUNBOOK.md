...
- **正式 Workstation**：Ubuntu AI 電腦，第一次安裝已包括文字 AI 同本地 RAG；日後亦
  可提供語音辨識同自家語音。
...
| 更新 RAG，或啟用 ASR／自家 TTS | [第 4 節](#4-管理-rag-更新或啟用語音功能) |
...
| workstationctl | Workstation 本機管理指令（經 Tailscale SSH 以 `sudo -u lmc-ai /usr/bin/python3 -m workstation.scripts.workstationctl <指令>` 執行）|
...
3. **Token 只輸入 `workstationctl pair` 嘅隱藏輸入欄。** 唔好放入 command 參數、
   截圖、log、issue 或文件。
4. **唔好公開任何 AI 服務 port。** Ollama 同 GPT-SoVITS 只可喺本機
...
- [ ] Workstation：RTX 3060 12GB（唔好買 8GB 閹割版）、最少 16GB RAM、500GB SSD。
...
- [ ] Developer 已批准 NVIDIA driver、kernel 同 CUDA 相容組合。唔好臨場追最新版。
  （RTX 5060 Ti 係 Blackwell 架構，driver 必須係 570 系或以上。）
- [ ] Developer 已批准 Ollama 版本。
...
- [ ] 包括文字模型及 RAG embedding 模型嘅 signed Ollama model inventory 已準備。
- [ ] 第一版 RAG v1 已由人手逐項核對，`documents.jsonl` 至少有一行有效資料，來源、
  授權、版本、撤回方法及內容 SHA-256 已記錄。
- [ ] RAG v1 已製成 immutable bundle，並已連同 release manifest 離線簽署。
- [ ] 已安排有人在場嘅 maintenance window，測 BIOS、冷開機、真正休眠、RTC 喚醒、
  斷電同非 Tailscale 網絡。

RAG v1 係第一次正式上線嘅必需項目，唔可以使用空白、placeholder 或未審核資料代替。
ASR model、GPT-SoVITS 同相關 private R2 則只喺準備啟用相應語音功能時先要準備。
...
#### 步驟 2.5：安裝 NVIDIA driver 同 Ollama

`.deb` 唔會安裝 driver 或 Ollama，兩樣都要喺呢一步先裝好，preflight 先會通過。

**A. NVIDIA driver**（用 Developer 已批准嘅版本，唔好揀最新）：

```bash
sudo apt update
sudo ubuntu-drivers list --gpgpu
sudo ubuntu-drivers install nvidia:DRIVER_VERSION
sudo reboot
```

先將 `DRIVER_VERSION` 換成 Developer 批准嘅版本號（例如 `570`）。RTX 5060 Ti
（Blackwell）要求 570 系或以上；Ubuntu 幫呢類卡揀 `-open` kernel module 變體係正常。
...
**成功標誌：** 見到正確 GPU 型號，driver 版本同步驟 1 基線記錄一致。見唔到 GPU 或
版本唔啱，停低通知 Developer，唔好自行試其他版本。

**B. Ollama**（用官方安裝方式，版本以 Developer 批准為準）：
...
官方安裝會建立 `ollama` service account 同 `ollama.service`。稍後步驟 3 安裝 `.deb`
時，系統會自動套用 override，將 Ollama 鎖喺 `127.0.0.1` 同
`/srv/lmc-ai/models/ollama`。

**唔好喺呢一步 `ollama pull` 任何模型。** 模型只可以喺步驟 6 經已簽署 inventory
安裝，呢個規則冇例外。

**成功標誌：** `ollama --version` 顯示批准版本；`systemctl status ollama` 係 active。
兩個版本號都記入步驟 1 嘅基線記錄。
...
```bash
ss -ltn | grep -E ':(11434|9880|3389)\b'
```

Ollama 同 GPT-SoVITS 只可見於 loopback。切勿公開 11434／9880，
...
3. 立即複製只顯示一次嘅 token。
4. 經 Tailscale SSH 執行配對，token 喺隱藏輸入欄貼上：

```bash
sudo -u lmc-ai /usr/bin/python3 -m workstation.scripts.workstationctl pair --name "LMC AI Workstation 1" --server-url https://<website-domain>
```

**成功標誌：**

- 指令輸出顯示配對成功，網站亦已接受新 Node 連線。
- 網站顯示新 Workstation online。
- 網站只得一個 enabled credential。

如果 token 或 WSS 未喺限時內獲網站接受，系統會還原舊設定。唔好不停重試或建立多個
credential，先去[第 2 節](#2-故障處理先停再查)。

#### 步驟 6：安裝文字及 RAG embedding 模型

經 Tailscale SSH 用 `workstationctl`：

1. 執行 `artifact-inspect`，先查看已簽署模型大小／雜湊。
2. 核對輸出嘅模型、精確大小、digest 同總下載量。
3. Developer 確認無誤後，執行 `model-approve` 安裝上述模型。
4. 等待下載完成同逐一 digest 驗證。
5. 執行 `full-health`。

完整 Workstation inventory 只由 `ai_model_config.py` 嘅
`lmc_ai_workstation_required_models()` 決定，包括可供使用者選擇嘅文字模型，同一個只供
RAG 建立向量嘅 embedding 模型。Embedding 模型唔會出現喺文字對話模型選單。本文件
唔抄寫 tag，避免文件過期。Workstation 開機、health check 同普通網站 request 都唔會
自行下載、升級或刪除模型。

12 GB 單 GPU profile 同一時間最多只容許一個 Ollama model resident；相同 model 可沿用
warm cache，轉 mode 或轉做 RAG 時會先卸載其他 model。所有文字生成 request 都會明確
要求 `ai_model_config.py` 定義嘅 context length，唔依賴 Ollama 按 VRAM 自行選擇預設值。
...
1. 執行 `artifact-inspect`。
2. 在 `rag_bundle` 核對 RAG v1 嘅 bundle ID、bytes 同 SHA-256。
3. Developer 對照安裝前記錄，確認呢個係已審核、已簽署而且非空白嘅
   `documents.jsonl` bundle。
4. 執行 `rag-install` 下載、建立及啟用 RAG。
5. 等 Workstation 驗證簽章同 hash，以已批准 embedding 模型建立本地 index，再原子切換
   `current`。
6. 執行 `full-health`。
7. 用一條只存在於 RAG v1 嘅測試問題核對檢索結果、`RAG:<id>` 引用同原文一致。

`documents.jsonl` 每行要填 `id`、`title`、`text`、`source_url`，詳細欄位規則同例子見
[`AI_TRAINING_RUNBOOK.md`](AI_TRAINING_RUNBOOK.md#8-未來單-gpu-微調開源辯論-llm)。
佢係每行一個 object 嘅 JSONL，唔係 JSON array；長文件要喺匯出前切成已審核、可獨立
理解同引用嘅段落。Workstation 禁止 embedding 自動截斷，任何過長段落會令新 bundle
安裝失敗並保留上一版。
Bundle 無檔案、零行、欄位不齊、ID 重複、網址不合規、簽章／hash 不符、embedding 模型
未獲批准，或者檢索測試無結果，都會 fail closed。

**成功標誌：** Full health 內 `rag` 同 `rag_probe` 均為 OK，active receipt 顯示 RAG v1，
測試問題命中正確段落及引用。

#### 步驟 8：設定每日休眠同喚醒

喺網站 Developer page「電源時間表」：
...
有 active job 時系統會每分鐘再檢查，唔會中斷工作。`workstationctl power-override
--until-epoch <epoch>` 嘅「臨時保持喚醒」只跳過 scheduled suspend，唔會改 RTC
排程或停止 active job。

#### 步驟 9：收集證據，先驗收後 Resume

跟[第 5 節](#5-真機驗收同證據)收集自動報告，再完成人手測試。全部 gate 合格先
Resume 同開放正式工作。RAG v1 未通過不得 Resume。Manager 亦會重新執行 full health，
RAG active receipt、index 或檢索 probe 有問題就保持 Drain。
...
- [ ] 已安裝非空白、已審核、已簽署嘅 RAG v1，active receipt、index integrity、
  retrieval、引用及 RAG rollback 測試通過。
...
- [ ] 如今次同時啟用 Voice／ASR／TTS，已完成相應 full health、私隱、保留期、
...
日常遙距操作用網站 Developer page；以下命令由 Developer 經 Tailscale SSH 執行，
係本機管理同網站唔可用時嘅後備（另有 `pair`、`artifact-inspect`、`model-approve`、
`rag-install`、`rag-rollback`、`dataset-prepare`、`training-start`、`cancel-operation`、
`set-update-channel`、`power-override`、`r2-probe` 等子指令）：
...
3. 用 `workstationctl status` 查看 WSS 係「已連線」定「未連線／等待心跳」。
4. 用 Terminal 命令查看 status 同最近 200 行 log。
...
4. 只有確定工作實際結果後，先喺網站 Developer page（或 `workstationctl
   ack_reconcile`）確認 reconciliation。
...
程式更新**唔會自動 poll**，只由 Developer 觸發：喺網站 Developer page 按「Update」，
或經 SSH 執行 `workstationctl update-check`。

1. 確認冇 active operation。
2. 更新頻道一般正式機用 Stable（改頻道用 `workstationctl set-update-channel`）。
3. 觸發一次更新。
...
只喺 Developer 已確認需要時，喺網站 Developer page 按「Release rollback」（或經 SSH
執行 `workstationctl rollback-previous`）一次。Rollback 會使用唯一 previous
slot，完成後仍要 full health 同人手確認先 Resume。唔好用 rollback 處理 driver、model、
RAG、dataset 或資料問題。

### 幾時一定要重新安裝 `.deb`

App update 只更新 `/opt/lmc-ai-workstation/releases/` 內程式。如果版本改到以下內容，
...
RAG v1 已經係首次 setup 必需項目。之後更新資料或啟用語音功能時，次序固定：

1. **保持現行文字 AI 及 RAG 正常**
2. **更新 RAG**
3. **啟用 ASR／Voice**
4. **啟用自家 TTS**

RAG v1 未完成會阻止首次 Resume。其後每一個更新或語音階段獨立驗收，未完成唔會取代
已驗收嘅現行 RAG。Workstation 失敗時會 fail closed，唔會靜默將私人內容交去
Gemini／OpenRouter。
...
首次 RAG v1 同其後每次更新，都只可使用網站已 review、可撤回、有 provenance 同已簽署
嘅非空白 bundle。

1. 網站 review／publish bundle。
2. 喺網站 Developer page 按「檢查 signed catalog」先查看已簽署 artifact（本機後備：
   `workstationctl artifact-inspect`）。
3. 按「安裝 RAG bundle」（本機後備：`workstationctl rag-install`）。
4. Workstation 會驗 signature／hash，喺新目錄建立 index。
5. 成功先原子切換 `current`；失敗會保留上一版。
6. 跑 full health，確認 retrieval 同引用。任何失敗都保留 Drain，同時保留上一個可用版本。
...
- 重啟 allowlisted Node／Ollama／GPT-SoVITS service
...
### 5.2 正式驗收時跑 Full health

```bash
sudo -u lmc-ai /usr/bin/python3 -m workstation.scripts.workstationctl full-health
```

Full health 只測 config 標示為 enabled 嘅 capability。正式 Workstation 嘅 RAG 必須保持
enabled，所以每次正式驗收都會測 `rag` 同 `rag_probe`。未啟用嘅 ASR 或 TTS 唔應該
拖垮文字及 RAG readiness。
...
- [ ] Ollama／GPT-SoVITS 只見於 loopback
- [ ] 冇 active job 時按時休眠
- [ ] 有 active job 時延遲休眠，完成後先休眠
- [ ] RTC wake 同 WSS reconnect
- [ ] Power loss／Manager restart reconciliation
- [ ] 所有文字模式同多輪對話
- [ ] `ollama ps` 顯示每個文字模式使用預期 context、`100% GPU`，同一時間最多一個 model
- [ ] RAG v1 active receipt、index integrity、retrieval、引用、更新同 rollback
- [ ] RAG 長段落會 fail closed；多批次 index build 完成後 embedding model 已卸載
...
Model inventory 只可列出 `lmc_ai_workstation_required_models()` 允許嘅 exact name，
並包括 registry digest 同實際 bytes。呢個 inventory 同時包括文字模型及專用 RAG
embedding 模型。RAG archive 根目錄要有非空白 `documents.jsonl`，每行必須通過 loader
欄位、唯一 ID、HTTPS 來源及大小限制。
...
- `workstation/scripts/workstationctl.py`：本機管理 CLI（配對、artifact、更新、診斷）。
...
Debug 或 health 失敗時用呢張表對照：每個 receipt 邊個寫、邊個讀、幾耐過期。
所有 receipt 都係細 JSON 檔，唔會包含 secret 或會員內容。

| Receipt | 位置 | 邊個寫 | 邊個讀 | 過期／失效 |
|---|---|---|---|---|
| `website.json` | state dir | Node（每次 WSS 連線／心跳） | Health `wss` check；privileged helper 喺 release 切換前確認網站連線新鮮 | 120 秒 |
| `r2-health.json` | state dir | `workstationctl r2-probe`（on-demand 診斷） | 只供人手診斷，唔係 health 需求 | 唔適用 |
| `release/release-state.json` | `/var/lib/lmc-ai-workstation/release/` | Privileged helper（install／switch／confirm／rollback） | `workstationctl status`／`rollback-previous` | 唔過期；每次 release 動作覆寫 |
| `models/active-receipt.json` | `/srv/lmc-ai/models/` | `model-approve`（signed Ollama inventory 安裝） | Health `ollama` check（digest 對照，防 model drift） | 唔過期；重新安裝 inventory 時覆寫 |
| `rag/<id>/component-receipt.json` + `rag/active-receipt.json` | `/srv/lmc-ai/rag/` | `rag-install`（驗簽成功後） | Health `rag` check；`rag-rollback` | 唔過期；原子切換 `current` 時更新 |
| `asr-preflight.json` | state dir | Full health ASR 真實轉錄通過後 | Shallow health `asr` check | 24 小時，或 ASR model identity 改變 |
| `gpt-sovits-preflight.json` | state dir | Full health TTS 真實合成通過後 | Shallow health `gpt_sovits` check | 24 小時，或 voice identity 改變 |
| `models/gpt-sovits/voices/<id>/active-receipt.json` | `/srv/lmc-ai/models/gpt-sovits/` | `approve_gpt_sovits_voice` CLI（人手批准 voice） | Workload 設定（揀 voice ID）、TTS runtime 驗證 artifact hash | 唔過期；只可由重新批准取代 |
| `checkpoints/<dataset>/training-result.json` | `/srv/lmc-ai/checkpoints/` | TTS training operation 完成時 | Developer 批准 voice 前人手核對 | 唔過期；只描述該 training run |
