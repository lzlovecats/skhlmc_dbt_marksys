...
5. **配對網站**：按第 4 節用一次性 node token 配對，等網站新鮮
   `hello.accepted` receipt；確認舊 node 已停用，新 node 未通過 acceptance 前唔好選為
   production node；
6. **安裝及審批 AI**：只裝 signed inventory 內嘅 Ollama model；完成 ASR benchmark
   及 receipt、GPT-SoVITS training／blind-listening approval、signed RAG atomic activation；
7. **設定電源**：按第 5 節設定每日 suspend／RTC wake；有人在場做一次 active-job
   delay、真正 suspend、RTC wake及 WSS reconnect；
8. **跑完整驗收**：按第 9 節產生 Ubuntu evidence、20-turn browser Voice latency
   report，並做 RDP、斷電、故障注入、update/rollback、retention 同 privacy smoke；
9. **先驗收、後啟用**：全部 gate 合格先喺網站揀新 Workstation node；保存 evidence、
...
- Mac RDP、Tailscale SSH、非 tailnet拒絕、cold boot、suspend／RTC wake、斷電恢復
  全部通過；
- ASR、RAG、GPT-SoVITS、direct-R2、更新／rollback、GPU lease及所有指定 failure
  injection 通過；
- `verify_voice_latency` 接受至少 20 個全部成功嘅 local warm turns；
- 網站顯示新 node online及正確 capabilities，「與自家AI練習」以使用者正／反方各
  完成一節，正方先行、fallback及完場評語都正確；
- logs、diagnostic bundle、browser report同 evidence 冇 token、signed URL、錄音、
  transcript或其他會員資料；
- 舊文字 node已停止／disable，新 node先正式被選用。

任何 gate 失敗，都先將新 node drain並保留舊 node選擇，唔可以繞過 health gate。
...
1. 網站 Developer console 建立 node，立即複製一次性 token；
2. 在本機 GUI 填 node 名、網站 URL、token。Token 只寫入 mode 600 credential
   file，GUI response／log／status 永不回傳。配對動作會以固定 service 名停止及
   disable 舊 `skhlmc-lmc-ai-node.service`，再 enable 新 node；新 unit 亦宣告
   `Conflicts=`，避免兩個 node 同時 claim。GUI 只有收到新 node 嘅新鮮
   `hello.accepted` receipt 先會報配對成功；token／WSS 未於 bounded deadline 被網站
   接受，就會還原舊 config、token 同原有兩個 unit 狀態。Developer 仍必須核對新 node
   已連線，先喺網站選用佢；
...
RTX 3060／VRAM／data filesystem、UFW／Tailscale／RTC preflight，同一次真 functional
full health（包括短期 R2 upload/download/delete）；報告唔會保存 token、prompt、錄音、
逐字稿或 signed URL：
...
Warm Voice latency 必須由 Mac browser 量度 end-to-end，而唔係用 Manager 內部 stage
...
呢個流程只建立輕量文字 node，唔提供 Manager、RAG、ASR、GPT-SoVITS 或 direct-R2。
...
1. 喺網站 Developer console 建立／更換自家 AI 電腦 credential，立即複製只顯示一次
   嘅 token；
2. 先停止正式 Workstation `lmc-ai-node.service`，確認佢已離線；
...
成功後喺 Developer console 核對新 WSS receipt、model profile、ready 同 draining 狀態。
...
然後先重新啟動正式 `lmc-ai-node.service`，確認網站只見一部可接單 Workstation。

### 10.4 Rotate、更新及故障檢查

- Rotate token 後舊 socket 會即時斷線；重新執行 `configure`，再 restart service；
- 更新 node code/dependencies 前先 drain，更新後重跑 pinned requirements、preflight 同
  install-service；
- model profile version 改變時，舊 node 會被 server 拒絕，必須先同步模型同程式；
- browser 對話只保存喺 browser，server 唔會 durable 保存 prompt 或 transcript；
- 本地 node 離線時 fail closed，唔會靜默轉去 Gemini／OpenRouter。
