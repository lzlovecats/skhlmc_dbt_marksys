...
| `schema.py` | 新環境 bootstrap schema及凍結中的歷史 idempotent retrofit | `CREATE IF NOT EXISTS`不是migration history；現有drift對齊後不再新增retrofit |
...
- 原有20個endpoint/request-path重複`CREATE INDEX IF NOT EXISTS`已全部移除，index definition由`schema.py`單一擁有。Startup仍間接執行`idx_ai_coach_prepare_usage_user_created` compatibility index；production本來缺少的index不可再由request偷偷建立，必須先用實際query plan證明價值，再加入成對versioned migration。
- 已由production及bootstrap共同擁有的request/lazy table/column DDL、process locks及相關dead tests已移除；AI Training亦不再重建五張shared TTS/LLM tables或重加已存在的`audio_sha256`／`r2_key`。RAG reindex不再由request安裝`vector` extension或ALTER embedding欄，未有正式migration會明確503。加強scanner追蹤兩個imported startup DDL collections後，runtime DDL由原基線76個直接source sites降至21個execution-reference sites。剩餘集中於typed-config過渡、production未落地的AI/resource tables及startup compatibility DDL；呢21處仍是P1未完成項，不可視作長期架構。
- `ai_coach_live_briefs`, `ai_coach_prepare_usage`, `projector_state` 已由`schema.py`統一擁有，並只在startup做一次相容建立，request path不再執行DDL；baseline已完成，下一步在staging reconciliation判斷佢哋與bootstrap有沒有definition drift。`tg_notification_queue`仍未在repo schema／active code找到owner，要在staging確認資料及外部consumer後才保留或移除。
