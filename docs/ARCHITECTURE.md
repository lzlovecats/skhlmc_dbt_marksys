...
Audit起始時production有41張public tables；同日完成typed config、migration ledger、四張resource guards及TTS privacy metadata後，現為47張public tables：46張application tables加內部`schema_migrations`。AI training audit migration `0004`仍刻意pending，等4.2.0新runtime上線後才正式套用。`schema.py`只負責未有migration ledger的新空database bootstrap；七張future AI tables只保留table identifiers／gated code path，沒有bootstrap DDL，production truth一律以migration ledger及catalog為準。

已確認事項：

- 所有47張public tables的RLS均為關閉；內部migration ledger已以table grants隔離，application tables的RLS啟用次序見 `docs/ROADMAP.md`，不可直接一鍵全開。
...
- Read-only reconciliation確認46張application tables中45張與bootstrap owner重疊；唯一production-only `tg_notification_queue`估算0 rows／24,576 bytes，但未排除外部writer前不會drop。Code-only共八張：pending audit及七張dataset/model/eval/RAG future tables；後七張已分三個bundle fail-closed，不再由request或bootstrap建立。
...
- `20260713_0004_provision_ai_training_audit`已完成forward/rollback驗證後安全rollback，production仍以`0003`為head。正式post-apply預期為48 tables／363 columns／141 constraints／88 indexes／0 RLS，checksum `13afcc9eb18f636e5cb3de47866b15a6e02607598ec8b2e80aedf4c083adafd7`；browser table/sequence privileges全為false。Consent grant/withdraw證據永久保留，其他training audit保留400日。
