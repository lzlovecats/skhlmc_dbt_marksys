...
- Read-only reconciliation確認42張application tables中41張有bootstrap owner；唯一production-only `tg_notification_queue`估算0 rows／24,576 bytes，但未排除外部writer前不會drop。Bootstrap另宣告12張production未存在的AI/resource tables；是否啟用要逐feature決定，不再由第一個request暗中建立。
...
- Runtime原有20個重複`CREATE INDEX IF NOT EXISTS`已全部移除，index definition由`schema.py`單一擁有。Production本來缺少的index不會因deploy後第一個request自動出現；必須先用實際query plan證明價值，再加入成對versioned migration。
