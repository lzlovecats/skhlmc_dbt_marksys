...
- 已由production及bootstrap共同擁有的request/lazy table/column DDL、process locks及相關dead tests已移除；AI Training亦不再重建五張shared TTS/LLM tables或重加已存在的`audio_sha256`／`r2_key`。加強scanner追蹤兩個imported startup DDL collections後，runtime DDL由原基線76個直接source sites降至23個execution-reference sites。剩餘集中於typed-config過渡、production未落地的AI/resource tables及startup compatibility DDL；呢23處仍是P1未完成項，不可視作長期架構。
