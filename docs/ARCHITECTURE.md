...
| `core/` | domain logic、parameterized SQL、provider/storage adapters | 不依賴 UI；所有 DB executor 明確注入；批量查詢避免 N+1 |
...
- `ai_coach_live_briefs`, `projector_state` 由 runtime自行建立；`tg_notification_queue` 未在 repo schema/active code找到 owner。三者要在版本化 migration第一期正式歸檔或移除。
...
2. `_ProxyDb.query()` 仍把結果 materialize成 Pandas DataFrame；新 hot path應直接使用 row mapping/streaming或 repository method，舊 code逐域替換。
