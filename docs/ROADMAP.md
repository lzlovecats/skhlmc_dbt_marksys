...
| RLS | Production現有42張public tables（新增`app_config`）全部未啟用 | 先建立非owner、NOBYPASSRLS runtime role及request transaction context |
...
- [ ] 為現有production狀態建立只作baseline的migration version；不要把現有42張表重新CREATE一次。
...
- [x] 把`projector_state`、`ai_coach_live_briefs`及`ai_coach_prepare_usage`加入`schema.py` owner，request path不再執行相關DDL；production baseline確認後再納入首批versioned migration。
