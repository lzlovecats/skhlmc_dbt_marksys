...
Render WebSocket只處理authenticated SDP／ICE signaling、roster generation、turn、timer、
final transcript及文字AI評判；不接受或輸出PCM、native Gemini audio、`peer_audio`、
...
1. 每部裝置由使用者按鍵啟動咪／播放、ICE connected、data-channel ping、remote audio
   packets／energy及8秒`zh-HK` final transcript實測。
2. Media失敗會阻止開始並提示轉Wi-Fi／流動數據；系統不會改經TURN或Render。
3. Transcript失敗仍可開始真人練習，但全房`judge_enabled=false`；中途terminal error、
   空commit或sequence缺漏亦會永久停用AI評判，timer及P2P聲音繼續。
4. P2P中斷會暫停server timer並只做一次10秒ICE restart；失敗就安全完場。
5. Signaling有room membership、roster generation、byte、rate及TTL界限；ICE不入log或DB。
...
**Gate P0-LIVE-P2P：**兩部可直連裝置完成preflight及完整練習；逐字稿失敗只停AI評判；
