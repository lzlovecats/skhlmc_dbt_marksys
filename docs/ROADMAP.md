...
並行、frame、房間容量／TTL、provider timeout等技術安全邊界。AI Coach錄音改為browser直傳
R2，再由Render bounded stream上載Google Files API分析；Mode A聲音改為STUN-only P2P，
Mode B完整退役。Production現況仍以上方已核實版本為準，本文件不授權deploy或secret變更。
...
| **P0** | `4.5.5`授權deploy後smoke、admin／SQL credential rotation | 運維／安全 | Repo已ready，production仍是`4.2.1`；本roadmap不授權deploy或secret變更 |
...
### P0.1 Mode A STUN-only P2P direct media（repo已實作，待真機／授權deploy驗證）

**最終架構：**聯機只保留兩位真人對真人的Mode A（Free De及完整Mock）。兩部browser以
Cloudflare public STUN建立WebRTC Opus mono P2P；不設TURN、SFU、trusted edge agent、
Cloudflare media adapter或Render audio fallback。P2P會向房內另一位已驗證委員披露ICE
網絡候選地址，威脅模型限定為trusted committee member。

Render control plane只處理authenticated SDP／ICE signaling、roster generation、turn、timer及
final transcript；完場後由主持經HTTPS手動觸發一次文字AI評判。不接受或輸出PCM、native Gemini audio、`peer_audio`、
`tts_audio`或其他room media。Mode B（多人一隊對AI）、server-owned Gemini room socket、
room Azure／custom TTS、barge-in及JIT AI room session已完整退役；Solo browser-direct
Free De／Mock、Solo Azure／custom TTS、Kiosk評判及一般TTS不受影響。

**開始及失敗規則：**

1. 每部裝置由使用者按鍵啟動咪／播放、ICE connected、data-channel ping及remote audio
   packets／energy實測；兩位都通過才開始聯機。測試後音軌立即停傳，正式練習只會在
   輪到本人並按「開始發言」後傳送，停止、暫停或完場會立即停傳。
2. Media失敗會阻止開始並提示轉Wi-Fi／流動數據；系統不會改經TURN或Render。
3. 不設開局逐字稿測試；發言逐字稿可用時逐段保存。系統截停會先要求browser提交最終
   逐字稿，逾時則保留server已收到文字並標示「可能不完整」。房間完結後只可由主持手動
   要求一次AI評價；前端及server都會先確認正反雙方各有至少一段逐字稿，未齊不會消耗該次機會。
   4GB保護門檻會同時停用房內Web Speech逐字稿及AI評判；評判呼叫不另設output token上限，
   但仍有2MiB回應上限、45秒timeout及一次性要求保護。
4. P2P中斷會暫停server timer並只做一次10秒ICE restart；失敗就安全完場。
5. Signaling有room membership、roster generation、byte、rate及TTL界限；lobby最多10分鐘，
   開始後Mock上限為預定流程加15分鐘，Free De為雙方完整時間額加15分鐘；Free De由正方開始，
   每次停咪後嚴格正反交替，一方用完時間先跳過；ICE不入log或DB。
6. 完場逐字稿及結果只在process記憶體保留15分鐘；同時最多保留8個完場房，AI評判同時最多2個workflow。

**下一個gate：**在獲授權deploy前完成Desktop Chrome自動回歸，以及Android Chrome、iOS
Safari、嚴格NAT、tab background、網絡切換及ICE restart真機矩陣。Deploy後核對room channel
zero audio bytes、Render control bandwidth、Free／Mock timer／bells／roster／結果權限，並確認
嚴格NAT會明確失敗而不會出現隱藏fallback。此gate不新增vendor或media runtime。

**Gate P0-LIVE-P2P：**兩部可直連裝置完成preflight及完整練習；雙方有逐字稿時可要求AI評價；
嚴格NAT在10秒restart後安全完場；Render room channel沒有audio bytes；Mode B UI不存在且
`mode=B`明確400。真機矩陣及production bandwidth量度通過前，repo改動不等於授權deploy。
...
- [ ] 新版本只在獲明確deploy授權後上線；上線後重做version／health、AI Training非白頁、
...
**Gate P0-A（`4.2.1`達成 2026-07-14）：**主要登入workflow已smoke，production無新增5xx、DB pool、WebSocket或quota異常。`4.5.5` deploy後smoke仍是open item。
...
- [x] 使用次數quota已退役；保留的system-wide bandwidth／R2 reservation、single-use token、upload ownership、video comment防spam及push device self-trimming各自以transaction advisory lock、unique constraint或`ON CONFLICT`收口。
...
2. [ ] P0-LIVE-P2P真機gate完成後，抽出Mode A餘下的room control／signaling module；Render PCM／room Gemini／room TTS relay已退役，P6只重構剩下的control plane。Solo browser-direct token／page接線維持獨立。
...
**Gate P6-PROXY：**route/alias及OpenAPI parity、headers、login/maintenance、Solo direct token／resumption／GoAway、Mode A control／signaling close/error、roster generation、system-wide limits及Render control-bandwidth測試全過；zero-media-through-Render regression、單worker Live／room smoke無退化。

### P6.3 其餘長期工作

- [x] 最小offline regression suite已重建（2026-07-14）；4.2.1基線為六個檔29個case，
  4.5.0已按同一原則擴充AI Coach、Live、resource及security回歸。每個case對應真實
