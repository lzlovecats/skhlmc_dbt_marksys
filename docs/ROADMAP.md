...
- [x] 移除所有已在production及bootstrap共同存在之request/lazy table/column DDL、相關process locks及dead tests；AI Training亦不再重建五張shared TTS/LLM tables或重加已存在的`audio_sha256`／`r2_key`。RAG reindex不再由request安裝`vector` extension或ALTER embedding欄，schema未正式provision會503 fail closed。Resource guard migrations套用後再移除R2 intent、bandwidth、practice及AI coach quota runtime create。Production catalog確認兩個legacy BYTEA已nullable後，亦移除最後兩個startup ALTER及無caller的`schema.MIGRATIONS/run_migrations`。Runtime DDL由原基線76個直接source sites降至12個，餘下只限typed-config過渡及未provision的AI Training schema；inventory gate鎖定允許檔案、DDL constants及12-site上限。
