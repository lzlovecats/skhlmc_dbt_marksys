...
| Versioned DB migrations | Production baseline ledger及repo SQL runner已建立；`schema.py`仍是bootstrap + retrofit混合 | 在staging restore對齊production/bootstrap drift，再寫第一批schema修正migration |
| RLS | Production現有42張application tables加內部`schema_migrations`，全部未啟用 | P1 staging forward/rollback通過後，先建立非owner、NOBYPASSRLS runtime role及request transaction context |
...
- [x] 加入`tools/reconcile_db_schema.py`作可重跑只讀inventory gate：42張application tables中41張與bootstrap owner重疊；唯一production-only是estimated 0 rows／24,576 bytes的`tg_notification_queue`；另有12張code-only AI/resource tables，已列入feature-by-feature migration清單。
- [x] Column-name reconciliation只發現已知migration差異：兩個legacy BYTEA、TTS consent四欄及recording四個audio metadata欄；type/default/constraint/index definition仍須在staging restore核對。
...
- [ ] 移除`api/`、`core/`及`deploy/`全部request/runtime `CREATE INDEX`：原有20個endpoint/request-path index已移除；startup仍間接執行`idx_ai_coach_prepare_usage_user_created` compatibility index。Production已存在及未存在者均不可再因第一個request偷偷改catalog，缺少的index要先以staging `EXPLAIN`證明才寫versioned migration。
- [x] 移除所有已在production及bootstrap共同存在之request/lazy table/column DDL、相關process locks及dead tests；AI Training亦不再重建五張shared TTS/LLM tables或重加已存在的`audio_sha256`／`r2_key`。RAG reindex不再由request安裝`vector` extension或ALTER embedding欄，schema未正式provision會503 fail closed。加強scanner追蹤兩個imported startup collections後，runtime DDL由原基線76個直接source sites降至21個execution-reference sites。餘下只限typed-config過渡、production未存在的AI/resource tables及startup compatibility DDL；inventory gate鎖定允許檔案、DDL constants及21-site上限，未完成staging ownership決定前禁止再增加。
...
- [x] 把`projector_state`、`ai_coach_live_briefs`及`ai_coach_prepare_usage`加入`schema.py` owner，request path不再執行相關DDL；production baseline已確認，下一步在staging reconciliation決定是否需要首批versioned migration。
