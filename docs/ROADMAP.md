...
- [x] 加入`tools/reconcile_db_schema.py`作可重跑只讀inventory gate：現時46張application tables中45張與bootstrap owner重疊；唯一production-only是estimated 0 rows／24,576 bytes的`tg_notification_queue`；另有8張code-only AI governance/eval/RAG tables，已列入feature-by-feature migration清單。
