...
- [x] 加入`tools/reconcile_db_schema.py`作可重跑只讀inventory gate：42張application tables中41張與bootstrap owner重疊；唯一production-only是estimated 0 rows／24,576 bytes的`tg_notification_queue`；另有12張code-only AI/resource tables，禁止靠request path無記錄地建立。
...
- [x] 移除`api/`、`core/`及`deploy/`全部request/runtime `CREATE INDEX`：原有20個index由`schema.py`單一擁有；production已存在及未存在者均不再因第一個request偷偷改catalog，缺少的index要先以staging `EXPLAIN`證明才寫versioned migration。
