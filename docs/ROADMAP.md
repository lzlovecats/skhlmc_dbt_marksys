...
- [x] 最小offline regression suite已重建（2026-07-14）：`tests/`六個檔29個case，覆蓋評分計算、Mock流程/響叮、Live render contract（rebrand污染及token簽名兩單真bug）、migration catalog（含`%`陷阱）、typed config coercion及quota時區邊界；CI與發布清單均會跑。新功能開發時按同一原則（每個case對應真實失敗模式）擴充。
