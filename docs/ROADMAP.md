...
- [x] 移除所有已在production及bootstrap共同存在之request/lazy table/column DDL、相關process locks及dead tests；AI Training亦不再重建五張shared TTS/LLM tables或重加已存在的`audio_sha256`／`r2_key`。加強scanner追蹤兩個imported startup collections後，runtime DDL由原基線76個直接source sites降至23個execution-reference sites。餘下只限typed-config過渡、production未存在的AI/resource tables及startup compatibility DDL；inventory gate鎖定允許檔案、DDL constants及23-site上限，未完成staging ownership決定前禁止再增加。
