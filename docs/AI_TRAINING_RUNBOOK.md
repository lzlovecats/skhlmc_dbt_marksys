...
credential，不應寫入 log、commit 或長期保存。Export response 會標示 `generated_at`／
`expires_at`，並以 `no-store` 及每次下載 nonce 防止 browser／proxy 重用舊 URL。建立工作目錄：
...
1. 由 AI 訓練頁面指定的 RAG／SFT Google 文件下載獲接納的原始內容；再次核對匿名化、
   使用權及撤回狀態。舊 database-backed LLM 提交／匯出流程已退役，server dataset
   snapshot 尚未 provision 前只作離線研究。
2. 先鎖定 20–50 條從未用作訓練的 eval cases，量度香港粵語自然度、引用準確、論證、
   追問、反駁、具體建議及私隱。
3. 先做 RAG。知識、規則及引用應留在可撤回、可更新的 approved corpus，不靠微調「背入」
   model。整理成 `documents.jsonl` 時，每行只可包含一個 JSON 物件，而且四個欄位都要
   存在：
   - `id`：非空白、穩定、全檔唯一的字串。建議包含類別、主題、版本及序號；
   - `title`：非空白標題，列明主題、資料類型及版本／年份；
   - `text`：非空白的獨立可理解段落，包括可引用內容、適用範圍、例外及限制；
   - `source_url`：公開來源填 HTTPS 網址，內部或非公開來源填空字串 `""`。即使留空，
     來源擁有人、授權依據、版本及撤回聯絡仍要保存在管理紀錄。

   每行不可加入其他欄位，亦不可把多個 JSON 物件直接連在同一行。例：

   ```json
   {"id":"fictional-library-hours-2099-001","title":"虛構示例｜星河中學圖書館延長開放時間｜2099","text":"本段為虛構示例。星河中學在 2099 年校內模擬辯論中，假設圖書館逢星期五延長開放一小時。此設定只供示範 JSONL 格式，並非真實校規。","source_url":""}
   ```

   `documents.jsonl` 唔係一個包住所有項目嘅 JSON array。匯出長文件前要由人按意思切成
   多個可獨立理解、可獨立引用嘅段落，每段各佔一行並使用穩定唯一 ID。每段必須完整放得
   入已批准 embedding model 嘅 context；Workstation 會以 `truncate=false` 建立向量，
   任何一段過長都會拒絕整個新 bundle，唔會靜默截走後半段，亦唔會在安裝時自動改 ID
   或引用。
4. 只有 RAG + prompt 已穩定但仍有一致風格/格式缺口，才由人把資料整理成
   `messages=[system,user,assistant]` instruction pairs。原始講稿或逐字稿不是天然 prompt/
   answer pair，不能直接塞入 SFT。整理後的 JSONL 建議命名為 `sft.jsonl`；目前未有
   Workstation SFT 匯入器，檔名並非程式強制，亦不可當作已可安裝的 runtime bundle。
   每行只可包含一個 JSON 物件，最外層只可包含 `messages`：
   - `messages`：陣列，現階段固定按次序放一個 `system`、一個 `user` 及一個
     `assistant` 訊息；
   - 每個訊息只可包含 `role` 及 `content`。`role` 是上述三個角色之一，`content`
     是非空白字串；
   - `system`：共同角色、語氣、資料不足及安全規則，不放會改變的賽規或個案事實；
   - `user`：真實、完整而且可回答的任務，並包括答案必需的上下文；
   - `assistant`：由辯論訓練人員逐字審核的理想答案，只訓練這段回覆。

   Google 文件內的「提交者」、「標題」及「來源」只供授權、追溯及撤回，不應寫入 SFT JSONL。
   需要引用的知識放入 RAG。例：

   ```json
   {"messages":[{"role":"system","content":"你是虛構校園的辯論練習助手。請以香港正體中文提供清晰、具體的分析，資料不足時須明確說明。"},{"role":"user","content":"以下是虛構辯題：星河中學應否在星期五延長圖書館開放時間？請示範如何比較正反雙方影響。"},{"role":"assistant","content":"這個虛構辯題可以從受益人數、影響程度、執行成本及替代方案四方面比較。正方須證明延長時間能實際改善學生溫習需要，反方則須交代人手及保安成本是否足以抵銷效益。最後應比較哪一方的影響較廣泛、較持久，而且更難由其他措施取代。"}]}
   ```
...
自家 AI inference runtime 只支援 `ai_model_config.py` 定義嘅 Gemma profile；未有新產品
決定前，離線微調實驗亦唔應另選第二個 model family。開始實驗時仍要重新核對所選
Gemma revision 嘅 model card、license、chat template、context 及 maintainer 狀態。
訓練 stack 可用 Hugging Face
