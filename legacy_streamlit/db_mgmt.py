...
_SCHEMAS = {
    "accounts — 委員會成員帳戶": [
        ...
        ("active_since",     "DATE",       "參與率起計日期"),
        ("last_login_at",    "TIMESTAMP",  "最後登入時間"),
        ("account_disabled", "BOOLEAN",    "180 日未登入停用標記"),
    ],
    "matches — 比賽場次": [
    ...
    "match_videos — 比賽片段連結": [
        ("id",            "SERIAL (PK)",          "自動編號"),
        ("match_id",      "TEXT (FK→matches)",    "所屬場次；舊比賽片段可留空"),
        ("match_label",   "TEXT",                 "舊比賽片段的手動場次名稱"),
        ("video_title",   "TEXT",                 "片段標題"),
        ("youtube_url",   "TEXT",                 "YouTube 片段連結"),
        ("standalone_topic_text", "TEXT",         "舊比賽片段的手動辯題"),
        ("standalone_pro_team",   "TEXT",         "舊比賽片段的手動正方隊名"),
        ("standalone_con_team",   "TEXT",         "舊比賽片段的手動反方隊名"),
        ("is_visible",    "BOOLEAN",              "是否在會員重溫頁顯示"),
        ("display_order", "INTEGER",              "顯示排序"),
        ("created_at",    "TIMESTAMP",            "建立時間"),
        ("updated_at",    "TIMESTAMP",            "最後更新時間"),
    ],
    "video_views — 影片觀看紀錄": [
        ("id",        "SERIAL (PK)",          "自動編號"),
        ("video_id",  "INTEGER (FK→match_videos)", "片段編號"),
        ("user_id",   "TEXT (FK→accounts)",   "觀看者帳號"),
        ("viewed_at", "TIMESTAMP",            "觀看時間"),
    ],
    "video_comments — 影片留言": [
        ("id",           "SERIAL (PK)",          "自動編號"),
        ("video_id",     "INTEGER (FK→match_videos)", "片段編號"),
        ("user_id",      "TEXT (FK→accounts)",   "留言者帳號"),
        ("comment_text", "TEXT",                 "留言內容"),
        ("created_at",   "TIMESTAMP",            "留言時間"),
    ],
    "video_votes — 影片勝負投票": [
        ("video_id",    "INTEGER (PK, FK→match_videos)", "片段編號"),
        ("user_id",     "TEXT (PK, FK→accounts)",        "投票者帳號"),
        ("vote_choice", "TEXT",                          "'pro' | 'con' | 'undecided'"),
        ("updated_at",  "TIMESTAMP",                     "更新時間"),
    ],
    "video_chapters — 影片章節": [
        ("video_id",      "INTEGER (PK, FK→match_videos)", "片段編號"),
        ("chapter_label", "TEXT (PK)",                     "章節名稱"),
        ("start_seconds", "INTEGER",                       "開始秒數"),
        ("display_order", "INTEGER",                       "顯示排序"),
        ("updated_at",    "TIMESTAMP",                     "更新時間"),
    ],
    "video_progress — 影片觀看進度": [
        ("video_id",         "INTEGER (PK, FK→match_videos)", "片段編號"),
        ("user_id",          "TEXT (PK, FK→accounts)",        "觀看者帳號"),
        ("watched_seconds",  "INTEGER",                       "上次觀看秒數"),
    ...
    "lateness_fund_records — 遲到罰款紀錄": [
        ("id",             "SERIAL (PK)",        "自動編號"),
        ("late_date",      "DATE",               "遲到日期"),
        ("member_user_id", "TEXT (FK→accounts)", "罰款者帳戶"),
        ("late_minutes",   "INTEGER",            "遲到分鐘"),
        ("paid_amount",    "NUMERIC(10,2)",      "已繳金額"),
        ("note",           "TEXT",               "備註"),
        ("created_by",     "TEXT (FK→accounts)", "記錄人帳戶"),
        ("created_at",     "TIMESTAMP",          "建立時間"),
        ("updated_at",     "TIMESTAMP",          "更新時間"),
    ],
    "lateness_fund_expenses — 遲到基金支出": [
        ("id",           "SERIAL (PK)",        "自動編號"),
        ("expense_date", "DATE",               "支出日期"),
        ("amount_hkd",   "NUMERIC(10,2)",      "支出金額"),
        ("note",         "TEXT",               "備註"),
        ("created_by",   "TEXT (FK→accounts)", "記錄人帳戶"),
        ("created_at",   "TIMESTAMP",          "建立時間"),
    ],
    "lateness_fund_periods — 遲到基金年度結餘": [
        ("year_label",       "TEXT (PK)",      "年度標籤，例如 2025-26"),
        ("opening_balance",  "NUMERIC(10,2)",  "Bal b/d"),
        ("note",             "TEXT",           "備註"),
        ("updated_at",       "TIMESTAMP",      "更新時間"),
    ],
    "debaters — 辯員名單": [
    ...
    "competition_registration_settings — 比賽報名設定": [
        ...
        ("registration_start",    "TIMESTAMP",    "報名開始時間"),
        ("registration_end",      "TIMESTAMP",    "報名截止時間"),
        ("updated_at",            "TIMESTAMP",    "最後更新時間"),
    ],
    "competition_registrations — 比賽報名紀錄": [
        ("id",                   "SERIAL (PK)", "自動編號"),
        ("competition_edition",  "INTEGER",     "比賽屆數"),
        ("team_name",            "TEXT",        "隊名"),
        ("main_debater_name",    "TEXT",        "主辯姓名"),
        ("first_deputy_name",    "TEXT",        "一副姓名"),
        ("second_deputy_name",   "TEXT",        "二副姓名"),
        ("closing_debater_name", "TEXT",        "結辯姓名"),
        ("contact_name",         "TEXT",        "聯絡人姓名"),
        ("contact_class",        "TEXT",        "聯絡人班別"),
        ("contact_phone",        "TEXT",        "聯絡電話"),
        ("status",               "TEXT",        "'submitted' | 'contacted' | 'confirmed' | 'withdrawn'"),
